...
def get_camera_position_per_frame(x_coordinates, y_coordinates, z_elevation, start_point, end_point, nr_frames):
    """
    Generates a list of camera positions to move linearly from start_point to end_point over a specified number of frames.

    Parameters:
    - x_coordinates, y_coordinates: Coordinates that define the range of the scene, used for scaling.
    - z_elevation: Elevation data of the scene, used to determine vertical range.
    - start_point, end_point: Starting and ending coordinates of the camera path.
    - nr_frames: Total number of frames in the animation.

    Returns:
    - List of tuples representing the camera position for each frame.
    """
    ...
    # Center the camera along the x and y coordinates of the scene
...
def get_rotating_camera_position(x_coordinates, y_coordinates, z_elevation, start_angle, end_angle, camera_height,
                                 camera_radius, nr_frames):
    """
    Generates a list of camera positions to create a rotating view around a central point in the scene.

    Parameters:
    - x_coordinates, y_coordinates: Scene coordinates used to determine x and y range.
    - z_elevation: Elevation data of the scene, used to calculate the z range.
    - start_angle, end_angle: Starting and ending angle for the rotation in degrees.
    - camera_height: Relative height of the camera above the scene.
    - camera_radius: Radius of the camera orbit.
    - nr_frames: Total number of frames in the animation.

    Returns:
    - List of tuples representing the camera position for each frame in the orbit.
    """
    # Calculate the range for each axis to scale the camera position
    x_range = abs(x_coordinates[-1] - x_coordinates[0])
    y_range = abs(y_coordinates[-1] - y_coordinates[0])
    ...
    for i in range(nr_frames):
        ...
        camera_x = math.sin(-radians) * max_range * camera_radius + np.mean(x_coordinates)
        camera_y = math.cos(-radians) * max_range * camera_radius + np.mean(y_coordinates)
